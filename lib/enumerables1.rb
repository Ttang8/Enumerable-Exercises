...
def array_sum_with_index(arr)
  ...
  new_arr = arr.map.with_index { |int, idx| int * idx }
  new_arr.reduce(:+)
end
...
def price_is_right(bids, actual_retail_price)
  under_bids = bids.select { |bid| bid <= actual_retail_price }
  under_bids.max
end
...
def num_factors(number)
  (1..number).count { |int| number % int == 0 }
end
...
def ordered_vowel_word?(word)
  vowels = %w(a e i o u)
  word_vowel_order = word.chars.select { |ch| vowels.include?(ch) }
  word_vowel_order == word_vowel_order.sort
end
...
def products_except_me(numbers)
  ...
  while i < numbers.length
    ...
    numbers.each_index do |idx|
      next if idx == i
      sub_array << numbers[idx]
    end

    ans << array_product(sub_array)
    i += 1
